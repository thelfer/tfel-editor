...
mkdir cmake
pushd cmake
tar -xvjf $build/build-check/autotools/build-autotools/qemacs-$pkg_name.tar.bz2
mkdir install-cmake
mkdir build-cmake
if test "x$fcheck" == "xno" ;
then
...
pushd build-cmake
cmake ../qemacs-$pkg_name/ -DCMAKE_INSTALL_PREFIX=$build/build-check/cmake/install-cmake
$make_exec 
# if test "x$pbuild" == "xyes" ;
# then
...
then
    ...
    cmake ../qemacs-$pkg_name/ -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$build/build-check/cmake/install-cmake-release
    $make_exec
    # if [ test "x$pbuild" == "xyes" ];
    # then
    #     make check ARGS="-j $nbproc"
    ...
    cmake ../qemacs-$pkg_name/ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$build/build-check/cmake/install-cmake-debug
    $make_exec
    # if [ test "x$pbuild" == "xyes" ];
    # then
    #     make check ARGS="-j $nbproc"
