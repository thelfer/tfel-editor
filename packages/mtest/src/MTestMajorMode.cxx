...
namespace tfel::gui {
  ...
  static void startMTest(EditorWidget& editor,
    ...
    if (t.isModified()) {
      ...
      QObject::connect(
          ...
          [&editor, &b, &t, scheme] { runMTest(editor, b, t, scheme); });
  ...
  static void insertBehaviour(TextEditBase& textEdit,
    ...
    if (!b->getIntegerParametersNames().empty()) {
      ...
      for (const auto& p : b->getIntegerParametersNames()) {
        ...
        tc.insertText("// @IntegerParameter '" + QString::fromStdString(p) +
                      "' " + QString::number(v) + ";\n");
      }
      tc.insertText("\n");
    }
    if (!b->getUnsignedShortParametersNames().empty()) {
      ...
      for (const auto& p : b->getUnsignedShortParametersNames()) {
        ...
        tc.insertText("// @UnsignedIntegerParameter '" +
                      QString::fromStdString(p) + "' " + QString::number(v) +
                      ";\n");
      }
      tc.insertText("\n");
    }
  ...
  MTestMajorMode::MTestMajorMode(EditorWidget& w, Buffer& b, TextEditBase& t)
      : CxxMajorMode(w, b, t) {
    QStringList keys;
  ...
  bool MTestMajorMode::keyPressEvent(QKeyEvent* const e) {
    ...
    if ((m == Qt::AltModifier) && (k == Qt::Key_M)) {
      startMTest(this->editor, this->buffer, this->textEdit, this->getScheme());
      return true;
    }
  ...
  QMenu* MTestMajorMode::getSpecificMenu() {
    ...
    QObject::connect(ra, &QAction::triggered, this, [this] {
      startMTest(editor, this->buffer, this->textEdit, this->getScheme());
    });
    auto* const tpa = m->addAction(QObject::tr("Plot results using TPlot"));
  ...
  const std::vector<std::string>& MTestMajorMode::getKeyWordsList() const {
  ...
  void MTestMajorMode::showResults() {
    ...
    if (n.isEmpty()) {
      this->editor.displayInformativeMessage(QObject::tr("no file name"));
      return;
    };
  ...
  static StandardFunctionCommandProxy<runMTest> runMTestProxy("run-mtest");
