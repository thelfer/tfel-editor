...
namespace tfel::gui {
  ...
  static void insertBehaviour(TextEditBase& textEdit,
    ...
    const auto fs = ((b->getBehaviourType() ==
    ...
    if (!b->getMaterialPropertiesNames().empty()) {
      tc.insertText("* material properties for behaviour '" + bd.behaviour +
                    "'\n");
      ...
      for (const auto& m : b->getMaterialPropertiesNames()) {
        ...
        auto mp_compare = [&mp](const MaterialPropertyDescription& mpd) {
          if (mpd.is<ConstantMaterialPropertyDescription>()) {
            const auto& cmpd = mpd.get<ConstantMaterialPropertyDescription>();
            return mp == cmpd.name;
          } else if (mpd.is<CastemMaterialPropertyDescription>()) {
            const auto& cmpd = mpd.get<CastemMaterialPropertyDescription>();
            return mp == cmpd.name;
          }
          return false;
        };
        ...
        if (pmpd != bd.material_properties.end()) {
          if (pmpd->is<ConstantMaterialPropertyDescription>()) {
            ...
            tc.insertText("MPROPS. '" + mp + "'. 'MODELE' = '" + cmpd.function +
                          "';\n");
  ...
  CastemMajorMode::CastemMajorMode(EditorWidget& w, Buffer& b, TextEditBase& t)
  ...
  bool CastemMajorMode::startCastem() {
    if (this->co == nullptr) {
      this->co = new ProcessInteractionFrame(this->editor, this->buffer);
  ...
  void CastemMajorMode::completeCurrentWord(TextEditBase& t, const QString& w) {
  ...
  QMenu* CastemMajorMode::getSpecificMenu() {
    ...
    auto* const src = m->addAction(QObject::tr("Send region to Cast3M"));
    src->setIcon(QIcon::fromTheme("system-run"));
    QObject::connect(src, &QAction::triggered, this,
                     &CastemMajorMode::sendRegionToCastem);
    auto* const sbc = m->addAction(QObject::tr("Send buffer to Cast3M"));
    ...
    auto* const imfmba = m->addAction(QObject::tr("Import MFM behaviour"));
  ...
  void CastemMajorMode::completeContextMenu(QMenu* const m,
    ...
    if (this->isCastemKeyWord(w.left(4))) {
      ...
      auto* const ha2 = new QAction(QObject::tr("Help on %1 (web)").arg(w), m);
  ...
  void CastemMajorMode::displayHelp(const QString& w, const QString& w2) {
    ...
    if (dir.exists()) {
      ...
      this->buffer.attachSecondaryTask(QObject::tr("notice %1").arg(w), nf);
