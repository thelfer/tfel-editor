...
namespace tfel::gui {
  ...
  static const MFrontExamples &getMFrontGalleryExamples() {
    static const MFrontExamples e(buildMFrontExamplesList("MFRONTGALLERYHOME"));
    return e;
  }
  ...
  static QString getDSLName(EditorWidget &editor, TextEditBase &t) {
    ...
    try {
      ...
      editor.displayInformativeMessage("getDSLName: " + QString(e.what()));
    } catch (...) {
  ...
  static void runMFront(EditorWidget &editor, Buffer &b, TextEditBase &t) {
    ...
    if (n.isEmpty()) {
      editor.displayInformativeMessage(QObject::tr("runMFront: no file name"));
      return;
    }
    QFileInfo fn(n);
    if ((!fn.exists()) || (!fn.isFile())) {
      editor.displayInformativeMessage(QObject::tr("invalid file name"));
      return;
    }
    ...
    const auto dsln_mkt = [&editor, &t] {
      ...
      const auto dsl = f.createNewDSL(name.toStdString())->getTargetType();
      if (dsl == mfront::AbstractDSL::MATERIALPROPERTYDSL) {
        return std::make_pair(name, MFrontOptionsDialog::MATERIALPROPERTY);
    ...
    auto optional_argument = [&args](const bool bvalue, const char *const opt) {
      if (bvalue) {
        args << opt;
      }
    };
  ...
  static void runMFrontDoc(EditorWidget &editor, Buffer &b, TextEditBase &t) {
    const auto n = t.getCompleteFileName();
    if (n.isEmpty()) {
  ...
  static void startMFront(EditorWidget &editor, Buffer &b, TextEditBase &t) {
    if (t.isModified()) {
  ...
  static void buildAnalyseFileMenu(
    ...
    if (!d.getCodeBlockNames().empty()) {
      ...
      for (const auto &cb : d.getCodeBlockNames()) {
        ...
        QObject::connect(vca, &QAction::triggered, o, [&w, &b, &t, cb, h] {
          auto &f = mfront::DSLFactory::getDSLFactory();
          try {
            auto *const cbb = new PlainTextEdit(w, b);
            auto dsl = f.createNewDSL(getDSLName(w, t).toStdString());
            dsl->analyseString(t.document()->toPlainText().toStdString());
            dsl->endsInputFileProcessing();
            const auto &nbd = dynamic_cast<mfront::AbstractBehaviourDSL &>(*dsl)
            ...
            b.setSecondaryTaskIcon(cbb, QIcon::fromTheme("mfront-icon"));
          } catch (std::exception &e) {
            debug(e.what());
          } catch (...) {
          }
  ...
  MFrontMajorMode::MFrontMajorMode(EditorWidget &w, Buffer &b, TextEditBase &t)
      : CxxMajorMode(w, b, t) {
  ...
  QMenu *MFrontMajorMode::getSpecificMenu() {
    ...
    auto online_ressource2 = [online_ressource](
                                 QMenu *const m, const QString &t,
                                 const QString &url, const QIcon i) {
    ...
    auto append_examples = [this, &m](const QString &label,
      ...
      if ((!emfm.properties.empty()) || (!emfm.behaviours.empty()) ||
        ...
        auto append = [this, &em](const QString &n,
    ...
    auto *const dm = m->addAction(QObject::tr("Generate documentation"));
    dm->setIcon(this->getIcon());
    QObject::connect(dm, &QAction::triggered, this, [this] {
      generateMFrontDocumentation(this->editor, this->buffer, this->textEdit);
    });
    // examples
  ...
  static StandardFunctionCommandProxy<runMFront> runMFrontProxy("run-mfront");
