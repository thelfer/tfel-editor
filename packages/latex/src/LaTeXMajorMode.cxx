...
namespace qemacs
{
  ...
  static int
  countNumberOfCurlyBraces(const QString& s)
  {
    int n = 0;
    ...
    : public QEmacsLineEdit
  {
    ...
    LaTeXInsertBlock(QEmacsWidget& p,
  ...
  private:
    struct Loader
    {
      Loader()
      {
	Q_INIT_RESOURCE(LaTeXModeResources);
      }
    };
    static Loader&
    getLoader()
  ...
  void
  LaTeXMajorMode::setSyntaxHighlighter(QTextDocument *const d)
  {
  ...
  void
  LaTeXMajorMode::completeCurrentWord(QEmacsTextEditBase& t,
				      const QString& w)
  {
  ...
  LaTeXMajorMode::startLaTeX()
  {
    ...
    QFileInfo fn(m);
    if(!fn.exists()){
      this->qemacs.displayInformativeMessage(QObject::tr("file '%1' does "
							 "not exist").arg(m));
      return;
    }
    QDir d(fn.dir());
  ...
  QMenu*
  LaTeXMajorMode::getSpecificMenu()
  {
    return nullptr;
  } // end of CppMajorMode::getSpecificMenu

  QIcon
  LaTeXMajorMode::getIcon() const
  {
	...
	if(cpos!=-1){
	  if(cpos!=pos){
	    if((posl>=pos)&&(posl<=cpos)){
	      this->completeContextMenuForMispelledWord(m,text.mid(pos,cpos-pos),
							posl-pos,bl.position()+pos);
	    }
	  }
	  ...
	  if(posl>=pos){
	    this->completeContextMenuForMispelledWord(m,text.mid(pos,text.size()-pos),
						      posl-pos,bl.position()+pos);
	  }
	  pos = l.size();
	}
  ...
  void
  LaTeXMajorMode::setSpellCheckLanguage(const QString& l)
  {
  ...
  void
  LaTeXMajorMode::completeContextMenuForMispelledWord(QMenu *m,
						      const QString& l,
						      const int p,
						      const int ap)
  {
	...
	if((p>npos)&&(p<pos)){
	  ...
	  if(!this->spellChecker.spell(w)){
	    ...
	    for(pa=this->suggestions.begin();
		pa!=this->suggestions.end();++pa){
	      delete *pa;
	    }
		...
		QAction *a = this->suggestions[i-1];
		cactions = m->actions();	
		if(cactions.isEmpty()){
		  m->addAction(a);
		} else {
  ...
  void
  LaTeXMajorMode::replaceMispelledWordBySuggestion(QAction * a)
  {
  ...
  bool
  LaTeXMajorMode::handleShortCut(const int k1,
				 const Qt::KeyboardModifiers m,
				 const int k2)
  {
    if((k1==Qt::Key_C)&&(m==Qt::ControlModifier)&&(k2==Qt::Key_E)){
      this->qemacs.setUserInput(new LaTeXInsertBlock(this->qemacs,
						     this->textEdit));
      return true;
    }
    return false;
  ...
  bool
  LaTeXMajorMode::mousePressEvent(QMouseEvent * const)
  {
    return false;
  ...
  bool
  LaTeXMajorMode::keyPressEvent(QKeyEvent * const e)
  {
    const int k                   = e->key();
  ...
  void
  LaTeXMajorMode::indentLine(const QTextCursor& tc)
  {
    int i;
    int di;
    bool b;
  ...
  void
  LaTeXMajorMode::getLineIndentation(const QTextCursor& tc_,
				     int& i,
				     int& di,
				     bool& b) const
  {
    ...
    QString l = LaTeXSyntaxHighlighter::stripComment(tc.block().text()).trimmed();
  ...
  int
  LaTeXMajorMode::getIndentationIncrement(const QString& line) const
  {
    int di = 0;
  ...
  void
  LaTeXMajorMode::indentRegion(const QTextCursor& tc)
  {
    if(!tc.hasSelection()){
      return;
    }
    ...
    b.beginEditBlock();
    while((b<=e)&&(!b.atEnd())){
      QString l = LaTeXSyntaxHighlighter::stripComment(b.block().text()).trimmed();
      di = this->getIndentationIncrement(l);
  ...
  static StandardQEmacsMajorModeProxy<LaTeXMajorMode> proxy("LaTeX",QVector<QRegExp>()
							    << QRegExp("^[\\w-0-9_\\.]+\\.tex")
							    << QRegExp("^[\\w-0-9_\\.]+\\.sty"));
