...
namespace tfel::gui {
  ...
  int MaterialPropertyModel::columnCount(const QModelIndex&) const { return 2; }

  QVariant MaterialPropertyModel::data(const QModelIndex& i, int r) const {
    if (!i.isValid()) {
      return QVariant();
    }
  ...
  QVariant MaterialPropertyModel::headerData(int i,
                                             Qt::Orientation o,
                                             int r) const {
    if (r == Qt::DisplayRole) {
  ...
  bool MaterialPropertyModel::sort1(const MaterialProperty& m1,
                                    const MaterialProperty& m2) {
    return m1.function < m2.function;
  }

  bool MaterialPropertyModel::sort2(const MaterialProperty& m1,
                                    const MaterialProperty& m2) {
    return m1.function > m2.function;
  }

  bool MaterialPropertyModel::sort3(const MaterialProperty& m1,
                                    const MaterialProperty& m2) {
    return m1.library < m2.library;
  }

  bool MaterialPropertyModel::sort4(const MaterialProperty& m1,
                                    const MaterialProperty& m2) {
    return m1.library > m2.library;
  }

  void MaterialPropertyModel::sort(int c, Qt::SortOrder o) {
  ...
  const QVector<MaterialProperty>& MaterialPropertyModel::materialProperties()
      const {
    return this->m;
  }

  void MaterialPropertyModel::setMaterialProperties(
      const QVector<MaterialProperty>& m_) {
    this->m = m_;
    this->beginResetModel();
    this->endResetModel();
  }
  ...
  MaterialPropertySelector::MaterialPropertySelector(EditorWidget& q,
    ...
    QObject::connect(openlib, &QPushButton::pressed, this,
    ...
    QObject::connect(lf, &LineEdit::textChanged, this,
                     &MaterialPropertySelector::updateMFMModel);
  ...
  void MaterialPropertySelector::updateMFMModel(const QString& m) {
    if (m.endsWith(".so")) {
      QFileInfo f(m);
      this->l->setText(
          QObject::tr("<b>Material properties available in '%1':</b>")
              .arg(f.fileName()));
    } else {
      this->l->setText(QObject::tr(
          "<b>Material properties available in MFrontMaterials:</b>"));
    }
