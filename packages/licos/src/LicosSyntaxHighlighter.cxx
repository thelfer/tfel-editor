...
namespace tfel::gui {
  ...
  void LicosSyntaxHighlighter::highlightBlock(const QString &text) {
    ...
    p = t.begin();
    if ((t.size() >= 7) && (p->value == "MaterialProperty") &&
        ((p + 1)->value == "<") && ((p + 2)->value == "castem") &&
        ((p + 3)->value == ">") && ((p + 4)->flag == Token::String) &&
        ((p + 5)->flag == Token::String) && ((p + 6)->flag == Token::String)) {
      auto *d = new LicosData;
      d->library = QString::fromStdString(
          (p + 5)->value.substr(1, (p + 5)->value.size() - 2));
      d->function = QString::fromStdString(
          (p + 6)->value.substr(1, (p + 6)->value.size() - 2));
      d->pos = (p + 6)->offset + 1;
    ...
    p = t.begin();
    if ((t.size() == 7) && (p->value == "MechanicalBehaviour") &&
        ((p + 1)->value == "<") &&
        (((p + 2)->value == "Isotropic") ||
         ((p + 2)->value == "Orthotropic")) &&
        ((p + 3)->value == ">") && ((p + 4)->flag == Token::String) &&
        ((p + 5)->flag == Token::String) && ((p + 6)->flag == Token::String)) {
      auto *d = new LicosData;
      d->library = QString::fromStdString(
    ...
    p = t.begin();
    if ((t.size() == 4) && (p->value == "MechanicalBehaviour") &&
        ((p + 1)->flag == Token::String) && ((p + 2)->flag == Token::String) &&
        ((p + 3)->flag == Token::String)) {
      auto *d = new LicosData;
      d->library = QString::fromStdString(
          (p + 2)->value.substr(1, (p + 2)->value.size() - 2));
      d->function = QString::fromStdString(
          (p + 3)->value.substr(1, (p + 3)->value.size() - 2));
      d->pos = (p + 3)->offset + 1;
      this->setCurrentBlockUserData(d);
