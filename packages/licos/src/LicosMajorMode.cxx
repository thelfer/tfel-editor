...
namespace qemacs {
  ...
  QString LicosMajorMode::getLicosFile(const QString &f) {
    ...
    if (checkFile(r, d + QDir::separator() + f)) { return r; }
    if (checkFile(r, d + QDir::separator() + "inputs/" + f)) { return r; }
    if (checkFile(r, d + QDir::separator() + "scripts/" + f)) { return r; }
    if (checkFile(r, d + QDir::separator() + "data/" + f)) { return r; }
    if (checkFile(r, f)) { return r; }
    if (checkFile(r, "inputs/" + f)) { return r; }
    if (checkFile(r, "scripts/" + f)) { return r; }
    if (checkFile(r, "data/" + f)) { return r; }
    ...
    if (userFiles != nullptr) {
      ...
      while (p != pe) {
        QString file = *p + "/" + f;
        if (checkFile(r, file)) { return r; }
        if (checkFile(r, *p + "/inputs/" + f)) { return r; }
        if (checkFile(r, *p + "/scripts/" + file)) { return r; }
        if (checkFile(r, *p + "/data/" + file)) { return r; }
        ++p;
      }
    }
    if (lpath.isEmpty()) {
      const QString root = lpath + "/share/licos/";
      if (checkFile(r, root + f)) { return r; }
      if (checkFile(r, root + "inputs/" + f)) { return r; }
  ...
  void LicosMajorMode::showMaterialWizard() {
    ...
    if (w.exec() == QDialog::Accepted) { w.writeMaterial(); }
  ...
  QString LicosMajorMode::getFileNameUnderCursor(const QTextCursor &tc,
    ...
    while (index >= 0) {
      ...
      if ((p > index) && (p < index + length)) { return i.cap(1); }
  ...
  bool LicosMajorMode::keyPressEvent(QKeyEvent *const e) {
    ...
    if (((m == Qt::ControlModifier) && (k == Qt::Key_M))
      ...
      if (this->import(this->textEdit.textCursor())) { return true; }
  ...
  void LicosMajorMode::comment() {
    ...
    if (tc.hasSelection()) {
      ...
      if (!tokenizer.isCStyleCommentOpened()) {
        ...
        uncomment = true;
        if (lines.isEmpty()) { uncomment = false; }
        ...
        while (p != pe) {
          ...
          if (p != stokens.begin()) {
            ...
            if (p2->line != p->line) {
              ...
              if (s > 0) { tc.insertText(QString(s, ' ')); }
            }
          }
          if ((w[0] == '/') && (w[1] == '/')) {
            // C++ comment
      ...
      if (tokenizer.getState() != LicosTokenizer::FAILED) {
        if (tokenizer.isCStyleCommentOpened()) { return; }
        if (tokenizer.begin() != tokenizer.end()) {
          const auto &t = *(tokenizer.end());
          if (t.flag == Token::Comment) { return; }
        }
      }
