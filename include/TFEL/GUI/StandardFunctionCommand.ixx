...
namespace tfel::gui {
  ...
  StandardFunctionCommandProxy<f>::StandardFunctionCommandProxy(
      const QString& n, const bool b)
      : name(n) {
    if (b) {
      auto& qcf = CommandFactory::getCommandFactory();
      qcf.addCommand(std::make_shared<StandardFunctionCommandProxy>(n, false));
    }
  }  // end of
  ...
  template <void (*f)(EditorWidget&)>
  Command* StandardFunctionCommandProxy<f>::getCommand(EditorWidget& w) const {
    return new StandardFunctionCommand(w, f);
  }  // end of StandardFunctionCommandProxy::getCommand

  template <void (*f)(EditorWidget&)>
  StandardFunctionCommandProxy<f>::~StandardFunctionCommandProxy() = default;
