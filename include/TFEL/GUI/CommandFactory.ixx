...
namespace tfel::gui {
  ...
  template <typename T>
  StandardCommandProxy<T>::StandardCommandProxy(const QString& n, const bool b)
      : name(n) {
    if (b) {
      auto& f = CommandFactory::getCommandFactory();
      f.addCommand(
          std::shared_ptr<CommandProxy>(new StandardCommandProxy(n, false)));
    }
  ...
  template <typename T>
  Command* StandardCommandProxy<T>::getCommand(EditorWidget& w) const {
    return new T(w);
  }  // end of StandardCommandProxy<T>::getCommand

  template <typename T>
  StandardCommandProxy<T>::~StandardCommandProxy() = default;
