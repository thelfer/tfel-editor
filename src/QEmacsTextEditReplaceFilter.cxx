...
namespace qemacs {
  ...
  bool QEmacsTextEditReplaceFilter::isOk() const {
    return this->bOK;
  }  // end of QEmacsTextEditReplaceFilter::isOk

  bool QEmacsTextEditReplaceFilter::findNext() {
    const auto tc = this->textEdit.textCursor();
    const auto nc =
        this->textEdit.document()->find(this->s1, tc, this->opts);
  ...
  bool QEmacsTextEditReplaceFilter::filterKeyPressEvent(
      QKeyEvent* const ev) {
    ...
    if (((ev->modifiers() == Qt::NoModifier) &&
      ...
      if ((ev->key() == Qt::Key_Y) || (ev->key() == Qt::Key_N)) {
        if (ev->key() == Qt::Key_Y) {
          ...
          tc.movePosition(QTextCursor::NextCharacter,
                          QTextCursor::MoveAnchor, -s1.size());
      ...
      if (ev->key() == Qt::Key_Exclam) {
        ...
        while (c) {
          tc.movePosition(QTextCursor::NoMove, QTextCursor::MoveAnchor);
          tc.movePosition(QTextCursor::NextCharacter,
                          QTextCursor::KeepAnchor, -s1.size());
          tc.removeSelectedText();
          tc.insertText(s2);
  ...
  QEmacsTextEditReplaceFilter::~QEmacsTextEditReplaceFilter() {
    ...
    if ((this->nb == 0) || (this->nb == 1)) {
      this->qemacs.displayInformativeMessage(
          QObject::tr("replaced %1 occurrence")
              .arg(QString::number(nb)));
    } else {
      this->qemacs.displayInformativeMessage(
          QObject::tr("replaced %1 occurrences")
              .arg(QString::number(nb)));
    }
  }
