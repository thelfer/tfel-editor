...
namespace tfel::gui {
  ...
  Command* CommandFactory::getCommand(const QString& n, EditorWidget& w) const {
  ...
  QStringList CommandFactory::getAvailableCommandsNames() const {
    QStringList keys;
    for (const auto& p : this->proxies) {
      keys.append(p.first);
    }
    return keys;
  }  // end of getAvailableCommandsNames
