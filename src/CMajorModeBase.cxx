...
namespace tfel::gui {
  ...
  CMajorModeBase::CMajorModeBase(EditorWidget& w, Buffer& b, TextEditBase& t)
  ...
  static void do_indent(EditorWidget& editor,
    ...
    cformat.start("clang-format",
                  QStringList()
                      << "-assume-filename=cxx" << ("-lines=" + lb + ':' + le)
                      << ("-style=file")
  ...
  void CMajorModeBase::indent(const QTextCursor& b,
    ...
    if (keepEmptyLine) {
      ...
      auto tc2 = tc;
      tc2.movePosition(QTextCursor::StartOfBlock, QTextCursor::MoveAnchor);
