...
namespace qemacs
{
  ...
  static int
  countNumberOfSpacesAtBeginningOfString(const QString& s)
  {
  ...
  bool
  QEmacsMajorModeBase::handleShortCut(const int,
  ...
  void
  QEmacsMajorModeBase::indentLine(const QTextCursor& c)
  {
    QTextCursor tc(c);
    tc.beginEditBlock();
  ...
  void
  QEmacsMajorModeBase::indentRegion(const QTextCursor&)
  {} // end of QEmacsMajorModeBase::indentRegion

  void
  QEmacsMajorModeBase::setSpellCheckLanguage(const QString&)
  {} // end of QEmacsMajorModeBase::setSpellCheckLanguage

  void
  QEmacsMajorModeBase::beginAndEndOfSelection(QTextCursor& b,
					      QTextCursor& e,
  ...
  void
  QEmacsMajorModeBase::findBeginningAndEndOfTheParagraph(QTextCursor& b,
							 QTextCursor& e) const
  {
    ...
    while((!b.atStart())&&(!found)){
      b.movePosition(QTextCursor::PreviousBlock,
		     QTextCursor::MoveAnchor);
    ...
    while((!e.atEnd())&&(!found)){
      found = e.block().text().trimmed().isEmpty();
      if(found){
	e.movePosition(QTextCursor::PreviousBlock,
		       QTextCursor::MoveAnchor);
	e.movePosition(QTextCursor::EndOfBlock,
		       QTextCursor::MoveAnchor);
      } else {
	e.movePosition(QTextCursor::NextBlock,
		       QTextCursor::MoveAnchor);
	e.movePosition(QTextCursor::EndOfBlock,
		       QTextCursor::MoveAnchor);
      }
  ...
  void
  QEmacsMajorModeBase::doIndentLine(const QTextCursor& tc,
				    const int i) const
  {
    QTextCursor nc(tc);
    nc.movePosition(QTextCursor::NoMove,
		    QTextCursor::MoveAnchor);
    ...
    nc.movePosition(QTextCursor::StartOfBlock,
    		    QTextCursor::MoveAnchor);
    if(i>0){
      nc.movePosition(QTextCursor::NextCharacter,
  ...
  QEmacsMajorModeBase::getSelectedLines(const QTextCursor& tc) const
  {
    ...
    QTextCursor end(e);
    end.movePosition(QTextCursor::StartOfBlock,
		     QTextCursor::KeepAnchor);
    while((b<end)&&(!b.atEnd())){
      QTextCursor be(b);
      be.clearSelection();
      be.movePosition(QTextCursor::EndOfBlock,
		      QTextCursor::KeepAnchor);
      if(be>=e){
	be = b;
	be.clearSelection();
	be.movePosition(QTextCursor::NextCharacter,
			QTextCursor::KeepAnchor,
  ...
  QEmacsMajorModeBase::comment(void)
  {
    ...
    if(cs.isEmpty()){
      this->qemacs.displayInformativeMessage(QObject::tr("no comment syntax defined"));
      return;
    }
	...
	while((!b.atEnd())&&(b.position()<pe+i)){
	  ...
	  if(l.mid(li).startsWith(cs)){
	    b.clearSelection();
	    b.movePosition(QTextCursor::NextCharacter,
			   QTextCursor::KeepAnchor,li+cs.size());
  ...
  int
  QEmacsMajorModeBase::positionInCurrentBlock(const QTextCursor& tc) const
  {
    QTextCursor b(tc);
    b.movePosition(QTextCursor::StartOfBlock,
		   QTextCursor::KeepAnchor);
  ...
  QStringList
  QEmacsMajorModeBase::findFiles(const QString& d,
				 const QString& f,
				 const int m,
				 const int s)
  {
