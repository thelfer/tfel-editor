...
namespace tfel{
  ...
  struct InsertRectangle : public CommandLine {
    InsertRectangle(TextEditBase& t, EditorWidget& p)
        : CommandLine(QObject::tr("String rectangle"), p),
          textEdit(t) {
  ...
  struct RectangleMode::RectangleModeLineEdit
      : CommandLine::CustomLineEdit {
    RectangleModeLineEdit(TextEditBase& t,
                          EditorWidget& q,
                          CommandLine& p)
        : CustomLineEdit(q, p),
  ...
  RectangleMode::RectangleMode(TextEditBase& t,
                                           EditorWidget& p)
      : CommandLine(QObject::tr("Ctrl-X r "), p, false),
        textEdit(t) {
  ...
  void RectangleMode::pasteRectangle() {
    ...
    for (auto p = RectangleMode::killedRectangle.cbegin();
      ...
      ++p;
      if ((p != RectangleMode::killedRectangle.end()) &&
          (bl.atEnd())) {
        bl.insertText("\n");
      }
    }
  ...
  void RectangleMode::killRectangle() {
    ...
    if (tc.hasSelection()) {
      ...
      while ((b.blockNumber() <= e.blockNumber()) && (!b.atEnd())) {
        ...
        RectangleMode::killedRectangle.push_back(
            ct.mid(posl, nb));
        if (ct.size() > posl + nb) {
          end = ct.mid(posl + nb);
        }
