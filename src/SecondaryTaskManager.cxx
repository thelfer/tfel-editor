...
namespace tfel::gui {
  ...
  SecondaryTaskManager::SecondaryTaskManager(const SecondaryTaskManager&) =
      default;
  SecondaryTaskManager::SecondaryTaskManager(SecondaryTaskManager&&) = default;
  ...
  void SecondaryTaskManager::setCurrentSecondaryTask(Buffer* const b,
                                                     QWidget* const w) {
    const auto pb = this->bufferTasks.find(b);
    if (pb == this->bufferTasks.end()) {
      return;
  ...
  void SecondaryTaskManager::attachSecondaryTask(Buffer* const b,
                                                 const SecondaryTask& t) {
  ...
  void SecondaryTaskManager::detachSecondaryTask(Buffer* const b,
                                                 QWidget* const w) {
    if ((w == nullptr) || (b == nullptr)) {
      return;
    }
    ...
    if (p != this->bufferTasks.end()) {
      ...
      if (pw != p->second.end()) {
        title = pw->title;
        debug("removing task:", title, "for buffer", b->getBufferName());
        p->second.erase(pw);
        if (p->second.empty()) {
  ...
  void SecondaryTaskManager::showSecondaryTask(Buffer* const b,
                                               QWidget* const w) {
    if ((w == nullptr) || (b == nullptr)) {
      return;
    }
  ...
  void SecondaryTaskManager::hideSecondaryTask(Buffer* const b,
                                               QWidget* const w) {
    if ((w == nullptr) || (b == nullptr)) {
      return;
    }
  ...
  const std::vector<SecondaryTask>& SecondaryTaskManager::getSecondaryTasks(
      Buffer* const b) {
