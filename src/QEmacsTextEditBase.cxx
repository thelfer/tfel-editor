...
namespace qemacs
{
  ...
  void
  QEmacsTextEditBase::mouseMoveEvent(QMouseEvent * e)
  {
    qDebug() << "pos : " << e->pos();
  }

  void
  QEmacsTextEditBase::setFocus()
  {
    auto w = this->widget();
    if(w!=nullptr){
      w->setFocus();
  ...
  protected:
	...
	if(b!=nullptr){
	  ...
	  QEmacsLineEdit *l;
	  l = new QEmacsTextEditBase::KillOtherBufferAndWriteFile(this->qemacs,
								  this->textEdit,
								  *b,this->file);
  ...
  protected:
	...
	this->qemacs.removeUserInput(this);
	QEmacsLineEdit *l;
	l = new QEmacsTextEditBase::OverWriteFile(this->qemacs,
						  this->textEdit,f);
	this->qemacs.setUserInput(l);
      } else {
	// check if another buffer does not visit this file
	QEmacsBuffer * b;
	b = this->qemacs.getBufferVisitingFile(f);
	if(b!=nullptr){
	  this->qemacs.removeUserInput(this);
	  QEmacsLineEdit *l;
	  l = new QEmacsTextEditBase::KillOtherBufferAndWriteFile(this->qemacs,
  ...
  bool
  QEmacsTextEditBase::SaveInput::isBlocking() const
  {
    return true;
  }

  void
  QEmacsTextEditBase::SaveInput::treatUserInput()
  {
    if(this->input->text()=="y"){
      this->textEdit.save();
    }
  ...
  QEmacsTextEditBase::GotoLine::GotoLine(QEmacsTextEditBase& t,
					 QEmacsWidget& p)
    : QEmacsLineEdit(QObject::tr("go to line :"),p),
      textEdit(t)
  {
  ...
  void QEmacsTextEditBase::setMajorMode(QEmacsMajorMode *const m)
  {
    ...
    if(this->mode!=nullptr){
      QObject::disconnect(this->mode,nullptr,
			  this,nullptr);
      delete this->mode;
      this->mode = nullptr;
    }
    this->qemacs.displayInformativeMessage(QObject::tr("setting major mode '%1'").arg(m->getName()));
    this->mode = m;
  ...
  void
  QEmacsTextEditBase::deleteContextMenuActions()
  {
    delete this->ua;
    this->ua = nullptr;
    delete this->ra;
  ...
  bool QEmacsTextEditBase::setKeyPressEventFilter(QEmacsTextEditKeyPressEventFilter * const f)
  {
    ...
    if(this->filter!=nullptr){
      this->qemacs.displayInformativeMessage(QObject::tr("a key press event filter already exists"));
      return false;
    }
    if(!f->isOk()){
      return false;
  ...
  void QEmacsTextEditBase::save()
  {
    ...
    const QString& af = QFileInfo(this->fileName).absoluteFilePath ();
    this->qemacs.displayInformativeMessage(QObject::tr("saving file '%1'").arg(af));
    QTextDocumentWriter writer(this->fileName);
    writer.setFormat("plaintext");
    if (writer.write(this->document())){
      this->document()->setModified(false);
      this->qemacs.displayInformativeMessage(QObject::tr("wrote file '%1'").arg(af));
    } else {
      this->qemacs.displayInformativeMessage(QObject::tr("saving file '%1' failed").arg(af));
    }
  ...
  QEmacsTextEditBase::writeFile()
  {
    ...
    QEmacsLineEdit *l = new QEmacsTextEditBase::WriteFile(this->qemacs,
							  *this,path);
  ...
  void
  QEmacsTextEditBase::writeFile(const QString& f)
  {
    // basic check
    QFileInfo fi(f);
    if(fi.exists()){
      if(!fi.isWritable()){
	QString msg(QObject::tr("file '%1' already exists and is not readable"));
	this->qemacs.displayInformativeMessage(msg.arg(f));
      }
    }
    this->setFileName(f);
  ...
  void
  QEmacsTextEditBase::keyPressEvent(QKeyEvent * e)
  {
  ...
  bool
  QEmacsTextEditBase::handleKeyPressEvent(QKeyEvent *e)
  {
  ...
  bool
  QEmacsTextEditBase::handleKeyPressEventWithQtShortCuts(QKeyEvent *e)
  {
    const int k                   = e->key();
	...
	case Qt::Key_Tab:
	case Qt::Key_Backtab:
	  this->insertCompletion(completer->popup()->currentIndex().data().toString());
	  return true;
	default:
	  break;
	}
    ...
    if(tc.position()==pc){
      ...
      // const bool ctrlOrShift = m & (Qt::ControlModifier | Qt::ShiftModifier);
      const bool hasModifier = m != Qt::NoModifier;
      QString completionPrefix = this->mode->getCompletionPrefix();
      if(hasModifier||(completionPrefix.length() < this->mode->getMinimalCompletionLength()) ||
	 e->text().isEmpty() ||
  ...
  bool
  QEmacsTextEditBase::handleKeyPressEventWithEmacsShortCuts(QKeyEvent *e)
  {
    const int k                   = e->key();
    const Qt::KeyboardModifiers m = e->modifiers(); 
    QTextCursor c = this->textCursor();
    ...
    if(((m == Qt::ControlModifier)&&(k==Qt::Key_X))&&(!this->ctrlx)&&(!this->ctrlc)){
      this->ctrlx = true;
      return true;
    }
    if(((m == Qt::ControlModifier)&&(k==Qt::Key_C))&&(!this->ctrlx)&&(!this->ctrlc)){
      this->ctrlc = true;
      return true;
    }
    if(this->ctrlx){
	...
	if(k==Qt::Key_1){
	  ...
	  this->qemacs.setUserInput(new QEmacsRectangleMode(*this,this->qemacs));
	...
	if(this->mode->handleShortCut(Qt::Key_X,m,k)){
      ...
      if(modifier.isEmpty()){
	this->qemacs.displayInformativeMessage(QObject::tr("unknown shortcut : Ctr-X - %1")
					       .arg(e->text()));
      } else {
	this->qemacs.displayInformativeMessage(QObject::tr("unknown shortcut : Ctr-X - %1-%2")
					       .arg(modifier).arg(e->text()));
      }
      return true;
    }
    if(this->ctrlc){
	...
	if(this->mode->handleShortCut(Qt::Key_C,m,k)){
      ...
      QString modifier = QEmacsTextEditBase::getModifier(*e);
      if(modifier.isEmpty()){
	this->qemacs.displayInformativeMessage(QObject::tr("unknown shortcut : Ctr-C - %1")
					       .arg(e->text()));
      } else {
	this->qemacs.displayInformativeMessage(QObject::tr("unknown shortcut : Ctr-C - %1-%2")
					       .arg(modifier).arg(e->text()));
      }
      return true;
    }
	...
	this->qemacs.displayInformativeMessage(QObject::tr("quit"));
      ...
      if(this->filter!=nullptr){
	this->qemacs.displayInformativeMessage(QObject::tr("a filter is already set"));
	return true;
      }
      // set a search user input
      // the QEmacsTextEditSearch automatically set itself as a filter for this
      this->qemacs.setUserInput(new QEmacsTextEditSearch(*this,this->qemacs));
	...
	return true;
      ...
      if(this->isReadOnly()){
	this->qemacs.displayInformativeMessage(QObject::tr("read only buffer"));	
      } else {
	...
	this->setTextCursor(c);
      ...
      QKeyEvent nev(QEvent::None,Qt::Key_PageDown,
		    Qt::NoModifier);
      QWidget *w = this->widget();
      w->removeEventFilter(this);
	...
	c.clearSelection();
      ...
      if(c.position()<=1){
	this->qemacs.displayInformativeMessage(QObject::tr("beginning of buffer"));	
	return true;
      }
      c.beginEditBlock();
      ...
      QKeyEvent nev(QEvent::None,Qt::Key_PageUp,
		    Qt::NoModifier);
      QWidget *w = this->widget();
      w->removeEventFilter(this);
      QCoreApplication::sendEvent(w,&nev);
      w->installEventFilter(this);
      return true;
    } else if(((m == (Qt::AltModifier|Qt::ControlModifier))&&(k==Qt::Key_Percent))||
	      ((m == (Qt::AltModifier|Qt::ControlModifier|Qt::ShiftModifier))&&(k==Qt::Key_Percent))){
      this->qemacs.setUserInput(new QEmacsTextEditQueryReplace(*this,this->qemacs));
	...
	tc.movePosition(QTextCursor::EndOfWord,
      ...
      return true;
	      ...
	      ((m == (Qt::AltModifier|Qt::ShiftModifier))&&(k==Qt::Key_Percent))){
      this->qemacs.setUserInput(new QEmacsTextEditQueryReplace(*this,this->qemacs));
    } else if(((m == Qt::AltModifier)&&(k==Qt::Key_Percent))||
	      ((m == (Qt::AltModifier|Qt::ShiftModifier))&&(k==Qt::Key_Percent))){
      this->qemacs.setUserInput(new QEmacsTextEditQueryReplace(*this,this->qemacs));
	...
	c.movePosition(QTextCursor::PreviousCharacter,QTextCursor::KeepAnchor,s);
	c.removeSelectedText();
  ...
  bool
  QEmacsTextEditBase::handleMousePressEvent(QMouseEvent *e)
  {
    if(this->filter!=nullptr){
