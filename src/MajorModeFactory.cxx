  ...
  namespace gui
{
  ...
  MajorMode* MajorModeFactory::getMajorModeByName(
      const QString& n,
      EditorWidget& w,
      Buffer& b,
  ...
  bool MajorModeFactory::hasMajorMode(
  ...
  MajorMode* MajorModeFactory::getMajorModeForFile(
      const QString& f,
      EditorWidget& w,
      Buffer& b,
      TextEditBase& t) const {
  ...
  QString MajorModeFactory::getMajorModeNameForFile(
      const QString& f) {
    for (const auto& p : this->proxies) {
      for(const auto& r : p.rexp){
        if (r.indexIn(f) >= 0) {
  ...
  void MajorModeFactory::addMajorMode(
      const MajorModeProxyPtr proxy,
  ...
  QIcon MajorModeFactory::getMajorModeIcon(
      const QString& n) const {
  ...
  QStringList
  MajorModeFactory::getAvailableMajorModesNames() const {
    QStringList n;
    for (const auto& p : proxies) {
    ...
    MajorModeFactory&
    MajorModeFactory::getMajorModeFactory() {
