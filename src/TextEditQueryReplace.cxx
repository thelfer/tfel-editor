...
namespace tfel::gui {
  ...
  TextEditQueryReplace::TextEditQueryReplace(TextEditBase& t, EditorWidget& p)
  ...
  TextEditQueryReplace::TextEditQueryReplace(TextEditBase& t,
                                             EditorWidget& p,
                                             const QString& s1_)
      : CommandLine(QObject::tr("query replace %1 with :").arg(s1_), p),
        textEdit(t),
        s1(s1_),
        stage(1) {
    this->input->setInputHistory(TextEditQueryReplace::getHistory());
  }
  ...
  TextEditQueryReplace::TextEditQueryReplace(TextEditBase& t,
      ...
      : CommandLine(QObject::tr("query replacing %1 with %2 "
        ...
        textEdit(t),
        stage(2) {
    auto* rf = new TextEditReplaceFilter(this->editor, this->textEdit, *this,
                                         s1_, s2_);
  ...
  void TextEditQueryReplace::treatUserInput() {
    ...
    if (this->stage == 0) {
      ...
      if (i.isEmpty()) {
        ...
        if (h.length() < 2) {
          this->cancel();
          this->editor.displayInformativeMessage(QObject::tr("empty entry"));
          return;
        } else {
          ...
          this->editor.setUserInput(new TextEditQueryReplace(t, q, i1, i2));
          this->textEdit.setFocus();
          return;
        }
      } else {
        ...
        this->editor.removeUserInput(this);
        this->editor.setUserInput(new TextEditQueryReplace(t, q, i));
        return;
      }
  ...
  void TextEditQueryReplace::addToHistory(const QString& i) {
    QSettings settings;
    auto h = settings.value("user input/query replace/history").toStringList();
    h.append(i);
    if (h.size() > 100) {
      h.pop_front();
    }
